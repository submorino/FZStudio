 ...
 * @n          [修改者]      2009-03-04  [描述]
 */
LPVOID SAFE_MAP::GetElement(DWORD dwKey)
{
	//多线程处理
	EnterCriticalSection(&m_cs);

	LPVOID lpValue = NULL;
	map<DWORD, LPVOID>::const_iterator iter = m_map.find (dwKey);
	if (iter != m_map.end())
		lpValue = iter->second;
 ...
 * @n          [修改者]      2009-03-04  [描述]
 */
void SAFE_MAP::PutElement(DWORD dwKey, LPVOID lpValue)
{
	//多线程处理
	EnterCriticalSection(&m_cs);
 ...
 * @n          [修改者]      2009-03-04  [描述]
 */
void SAFE_MAP::RemoveAt(DWORD dwKey)
{
	//多线程处理
	EnterCriticalSection(&m_cs);

	map<DWORD, LPVOID>::const_iterator CIT = m_map.find (dwKey);
	if (CIT != m_map.end())
		 m_map.erase(dwKey);
...
_int64 CppSQLite3Query::GetBigIntField(int nField, _int64  nNullValue)
{
...
_int64 CppSQLite3Query::GetBigIntField(const char* szField, _int64  nNullValue)
{
	int nField = FieldIndex(szField);
...
bool  CppSQLite3DB::Open(const char *szFile, const char* szKey)
{
	...
	if (szKey != NULL && szKey != "")
	{
		...
		{
			this->Close();
			throw CppSQLite3Exception(CPPSQLITE_ERROR, "key is not right!", DONT_DELETE_MSG);
		}
...
int CppSQLite3DB::Cmd(const char *fmt, ...)
{
    ...
    if (fmt == NULL)
    {
		throw CppSQLite3Exception(CPPSQLITE_ERROR, "SQL Statement is empty", DONT_DELETE_MSG);
	    return 0;
	}
	std::string temp = fmt;
	...
	if(head_word == "select")
	{
	   ...
	   {
		// no rows
		   m_mapRes.PutElement (::GetCurrentThreadId (), new CppSQLite3Query(m_pDB, pVM, true/*eof*/));
		 
	   }
    	else if (nRet == SQLITE_ROW)
	   {
		// at least 1 row
		   m_mapRes.PutElement (::GetCurrentThreadId (), new CppSQLite3Query(m_pDB, pVM, false/*eof*/));
...
int  CppSQLite3DB::Cmd(const char* fmt, const va_list args)
{
...
{
	CppSQLite3Query* pResultSet = (CppSQLite3Query*)m_mapRes.GetElement(::GetCurrentThreadId ());
	if (pResultSet)
	{
		pResultSet->Finalize();
		delete pResultSet;
	...
	m_mapRes.RemoveAt (::GetCurrentThreadId ());
}
...
{  
	CppSQLite3Statement* pStmt = (CppSQLite3Statement*)m_mapStmts.GetElement (::GetCurrentThreadId ());
	if (pStmt)
	{
		pStmt->Finalize ();
		delete pStmt;
	...
	m_mapStmts.RemoveAt (::GetCurrentThreadId ());
...
CppSQLite3Statement* CppSQLite3DB::CheckStmt()
{
	...
	{
		throw CppSQLite3Exception(CPPSQLITE_ERROR,
								"NULL CppSQLite3Statement Pointer",
								DONT_DELETE_MSG);
	}
	return pStmt;
}
