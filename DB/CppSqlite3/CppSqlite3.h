...
// 成员函数
	LPVOID GetElement(DWORD dwKey);                   //根据关键字获取值
	void   PutElement(DWORD dwKey, LPVOID lpValue);   //添加项
	void   RemoveAt(DWORD dwKey);                     //删除项
...
private:
	std::map<DWORD, LPVOID> m_map;
	CRITICAL_SECTION        m_cs;
	std::map<DWORD, LPVOID>::iterator m_iter;
};
...
// 成员函数
	...
	_int64 		GetBigIntField(int nField, _int64  nNullValue=0);  //获取整型字段值
	_int64 		GetBigIntField(const char* szField, _int64  nNullValue=0);
...
//========================CppSQLite3Table=======================
// 数据库静态查询类
class CPPSQLITE3_API CppSQLite3Table
{
public:
...
//=========================CppSQLite3Buffer=========================
//缓冲区处理类
class  CPPSQLITE3_API CppSQLite3Buffer
{
public:
// 构造函数与析构函数
    CppSQLite3Buffer();
...
//=========================CppSQLite3Binary=======================
//二进制编码处理类
class  CPPSQLITE3_API CppSQLite3Binary
{
public:
// 构造函数与析构函数
    CppSQLite3Binary();
